    ...
    courseId = parent.window.document.location.href.substr(_index + 1); // this gets the current courseId of the course the student is currently in
		...
		function goToPage() {
            ...
            document.getElementById("DisplayPgNum").innerHTML = "Page Number:" + "&nbsp" + pageNum + "/" + lastpagenumber;
		...
		function SaveClose(message) {
			...
			SaveSpendTime();
            ShowCloseButton();
			$("body").html("<br/><center><h1 style='color:white'>" + message + "</h1></center>");
		...
		function getTimeSpent(){
            ...
            $.ajax(settings).done(function (response) {
				...
				SaveClose("ThThere was an issue loading your course. Please close all internet browsers and try again. If you continue to receive this error please reach out to our student support team (904.999.4923 or support@xcelsolutions.com) and provide error code 7728.");
            });
			
			var convertTimeTicks = "<br/><span style='color:white'>" + Math.floor(((TimeSpentTicks/10000000)/60)/60) + "</span> Hours: <span style='color:white'>" + Math.floor(((TimeSpentTicks/10000000)/60) % 60) + "</span> Minutes : <span style='color:white'>" + Math.floor((TimeSpentTicks/10000000) % 60) + "</span> Seconds</br>";
